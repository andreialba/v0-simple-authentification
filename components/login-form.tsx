...
function LastUsedBadge() {
  return (
    <span className="absolute -top-2 -right-2 flex items-center gap-1 rounded-full bg-neutral-900 px-2 py-0.5 text-[10px] font-medium text-white">
...
export function LoginForm() {
  ...
  const [lastUsed, setLastUsed] = useState<LoginMethod>(null)

  useEffect(() => {
    const stored = localStorage.getItem("lastUsedLogin") as LoginMethod
    if (stored) setLastUsed(stored)
  }, [])

  const handleLogin = (method: LoginMethod) => {
    if (method) {
      localStorage.setItem("lastUsedLogin", method)
      setLastUsed(method)
    }
      ...
      >
        ...
        <div className="space-y-2">
          ...
          <input
            ...
            className="w-full rounded-lg border border-border bg-card px-4 py-3 text-sm text-card-foreground focus:border-ring focus:outline-none focus:ring-2 focus:ring-ring/20"
          />
        </div>
